...
Vsa obdelava podatkov se odvija na enem prenosnem računalniku. Zajemanje slik poteka sočasno v ločenih procesih. Računalnik ima dve fizični procesni jedri in štiri niti. 
...
Uporabil sem štiri Axis 215 PTZ IP kamere. Kamere so povezane v zvezdišče na ethernet omrežju, zvezdišče pa je povezano z brezžičnim usmerjevalnikom. Axis 215 PTZ so varnostne IP kamere namenjene primarno nadzoru okolja. Prednost kamer je, da so zmožne v realnem času preko lokalnega omrežja posredovati do 30 slik na sekundo pri ločljivosti 704 x 576. Slabost pa je ta, da varnostne kamere niso namenjene za simultano zajemanje slike in ne podpirajo skupinskega prožilca. Največja ločljivost je 704 x 576, kar je relativno malo za namen pozicioniranja objekta s premerom 4 cm v \textbf{X x X} velikem prostoru.

Specifikacije kamere:
….

Axis kamere imajo CGI (common gateway interface) vmesnik, ki omogoča nadzor funkcionalnosti preko HTTP protokola. Vsak model kamere podpira različen nabor ukazov, katerih spisek lahko dobimo z GET zahtevkom na http://<ip-kamere>/axis-cgi/com/ptz.cgi?info=1.

Označevalnik je v osnovi oranžna žogica za namizni tenis. Da pa bi sprememba svetlobe čim manj vplivala na natančnost zazanavanja označevalnika v sliki, sem v žogico vgradil modro svetlobno diodo (LED). Na sliki je označevalnik izrazito rumene barve.
