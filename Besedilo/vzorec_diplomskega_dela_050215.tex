...
\subsection{Zaznavanje označevalnika}\label{markersection}
...
Za zajemanje statičnih slik iz kamer sem uporabil Node.js. Node.js je JavaScript pogon z veliko različnimi moduli. Node.js arhitektura je po zasnovi asinhrona, kar pri vhodno/izhodnih operacijah močno pohitri sistem.
...
Za implementacijo glavnega sistema pa sem uporabil Python s knjižnicami Numpy, SciPy ter OpenCV. Morda se zdi Python za realnočasovni sistem slaba izbira, vendar je s pravilno uporabo knjižnic odlično orodje za hitri razvoj. Funkcije v knjižnicah so zaradi hitrosti implementiranje v njižjenivojskih jezikih kot so C/C++. Knjižnica Numpy je namenjena računanju z matrikami, SciPy pa je njena razširitev. OpenCV (Open Computer Vision) pa je namenjen za probleme umetnega zaznavanja in pri svoji implementaciji uporablja strukture Numpy-ja. 
\textbf{verzije programov in knjižnic}
...
Za zaznavanje označevalnika uporabljam postopek opisan v poglavju \ref{markersection}. Za prag sem določil RGB interval, ker je označevalnik izrazito rumene barve. Spodnja meja intervala je $[200 \ 200 \ 0]$, zgornja pa $[255, 255, 180]$. Spodaj so prikazane slike posameznih faz zaznavanja.
